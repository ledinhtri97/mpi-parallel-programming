...
#define STARTSIZE     131072
#define ENDSIZE       1048576
#define INCREMENT     2
#define ROUNDTRIPS    5
...
if (rank == 0) {
  ...
  printf("\n******************** MPI Bandwidth Test ********************\n");
  printf("Message start size= %d bytes ~[%d KB]\n",start, start/1024);
  printf("Message finish size= %d bytes ~[%d Kb]\n",end, end/1024);
  printf("Incremented by %d times bytes per iteration\n",incr);
...
if (rank < numtasks/2) {
  for (n=start; n<=end; n=n*incr) {
    ...
    if (rank == 0) {
      ...
      printf("---Message size: %4d [KB] ---\t[best]\t\t[avg]\t\t[worst] (MB/sec)\n",n/1024);
